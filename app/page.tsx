...
import { maintenancePatternById, maintenancePatterns } from "@/scenarios/templates";
import {
  compileIntervention,
...
const navItems: { id: View; icon: string; label: string; description: string }[] = [
  ...
  { id: "scenarios", icon: "▦", label: "Systems", description: `Explore ${scenarios.length} bounded systems` },
...
export default function Home() {
  ...
  const selectTemplate = (templateId: string) => {
    const firstSystem = scenarios.find((item) => item.system.maintenancePatternId === templateId);
      ...
      <div className="app-main">
        <header className="topbar">
          ...
          <div className="laboratory-selectors">
            <label className="scenario-select"><span>Maintenance pattern</span><select aria-label="Select a maintenance pattern" value={template.id} onChange={(event) => selectTemplate(event.target.value)}>{maintenancePatterns.map((item) => <option value={item.id} key={item.id}>{item.title}</option>)}</select></label>
            <label className="scenario-select"><span>Bounded system</span><select value={scenarioId} onChange={(event) => selectScenario(scenarioById[event.target.value])}>{scenarios.filter((item) => item.system.maintenancePatternId === template.id).map((item) => <option value={item.id} key={item.id}>{item.system.shortTitle} · {item.watchlistTier}</option>)}</select></label>
          </div>
...
function SystemDefinitionPanel({ scenario, template, protocol, interventionPlan }: { scenario: ScenarioDefinition; template: SystemTemplateDefinition; protocol: ScenarioProtocolDefinition; interventionPlan: InterventionPlanDefinition }) {
  ...
  return <details className="system-definition-panel" aria-label="Simulation setup">
    ...
    <div className="system-definition-body">
      <div className="model-flow five" aria-label="Maintenance pattern leads to the selected bounded system, scenario conditions, changes to test, and run result"><b>Maintenance pattern</b><i>→</i><b>Bounded system</b><i>→</i><b>Scenario conditions</b><i>→</i><b>Changes to test</b><i>→</i><b>Run result</b></div>
      <div className="system-definition-grid">
        <article><small>MAINTENANCE PATTERN</small><p><strong>{template.title}</strong> {template.stateArchetype}</p></article>
        <article><small>DOMAIN & DYNAMIC TRAITS</small><p><strong>{system.domain}</strong><br />{system.dynamicTraits.map((trait) => trait.replaceAll("-", " ")).join(" · ")}</p></article>
...
function ScenarioLibrary({ filter, setFilter, selectScenario }: { filter: string; setFilter: (value: string) => void; selectScenario: (scenario: ScenarioDefinition) => void }) {
  const isPatternFilter = maintenancePatterns.some((item) => item.id === filter);
  const filtered = filter === "All" ? scenarios : filter === "Featured" ? scenarios.filter((item) => item.featured) : isPatternFilter ? scenarios.filter((item) => item.system.maintenancePatternId === filter) : tierOrder.includes(filter as (typeof tierOrder)[number]) ? scenarios.filter((item) => item.watchlistTier === filter) : scenarios.filter((item) => item.system.domain === filter);
  const filters = ["All", "Featured", ...tierOrder, ...scenarioCategories];
  return <div className="page-view"><section className="page-hero"><div className="eyebrow"><span>Educational systems laboratory</span><span>{maintenancePatterns.length} maintenance patterns</span><span>{scenarios.length} bounded systems</span><span>{featuredSystemCount} featured systems</span></div><h1>Choose how a system maintains viability.<br /><em>Then test a bounded example.</em></h1><p>Maintenance patterns describe the recurrent work that keeps a system viable. Each example separately declares its domain, operator, boundary, population, time horizon, dynamic traits, two observable cycles, and real-world parameter meanings. Scenario conditions and planned responses can then change without redefining the system.</p></section><section className="template-library" aria-labelledby="template-library-title"><div className="section-heading"><div><h2 id="template-library-title">Maintenance patterns</h2><p>Choose the recurrent maintenance problem you want to study.</p></div><button onClick={() => setFilter("All")}>Show all systems</button></div><div className="template-grid">{maintenancePatterns.map((item) => <button key={item.id} className={filter === item.id ? "active" : ""} onClick={() => setFilter(item.id)}><span>{scenarios.filter((system) => system.system.maintenancePatternId === item.id).length} systems</span><h3>{item.title}</h3><p>{item.summary}</p><small>{item.learningQuestions[0]}</small></button>)}</div></section><div className="filter-row" role="group" aria-label="Filter systems">{filters.map((item) => <button className={filter === item ? "active" : ""} key={item} onClick={() => setFilter(item)}>{item in tierLabels ? `${tierLabels[item as keyof typeof tierLabels]} · ${watchlistCounts[item as keyof typeof watchlistCounts]}` : item === "Featured" ? `Featured · ${featuredSystemCount}` : item}</button>)}</div><p className="filter-result" aria-live="polite">Showing {filtered.length} system{filtered.length === 1 ? "" : "s"}{isPatternFilter ? ` using ${maintenancePatternById[filter as keyof typeof maintenancePatternById].title.toLowerCase()}` : ""}</p><div className="library-grid">{filtered.map((item) => <ScenarioCard key={item.id} scenario={item} onClick={() => selectScenario(item)} />)}</div></div>;
}
...
function ScenarioCard({ scenario, onClick, compact = false, active = false }: { scenario: ScenarioDefinition; onClick: () => void; compact?: boolean; active?: boolean }) {
  ...
  return <button className={`scenario-card ${compact ? "compact" : ""} ${active ? "active" : ""}`} onClick={onClick} style={{ "--scenario-accent": scenario.accent } as React.CSSProperties}><div className="scenario-art"><span>{scenario.icon}</span><i /><i /><i /></div><div className="scenario-card-body"><div><span className="category-pill">{scenario.system.domain}</span><span>{maintenancePatternById[scenario.system.maintenancePatternId].title}</span><span className={`tier-label tier-${scenario.watchlistTier}`}>{scenario.watchlistTier} outlook</span>{scenario.featured && <span>Featured</span>}</div><h3>{scenario.system.shortTitle}</h3><p><strong>{defaultProtocol.title}.</strong> {defaultProtocol.summary}</p>{!compact && <dl><div><dt>Pattern</dt><dd>{maintenancePatternById[scenario.system.maintenancePatternId].stateArchetype}</dd></div><div><dt>Dynamic traits</dt><dd>{scenario.system.dynamicTraits.map((trait) => trait.replaceAll("-", " ")).join(" · ")}</dd></div><div><dt>Operator</dt><dd>{scenario.system.operator}</dd></div><div><dt>Boundary</dt><dd>{scenario.system.boundary}</dd></div><div><dt>Population / rule</dt><dd>{scenario.system.population} {scenario.system.aggregation}</dd></div><div><dt>Minor cycle θ</dt><dd>{scenario.cycles.minor.label}</dd></div><div><dt>Major cycle φ</dt><dd>{scenario.cycles.major.label}</dd></div><div><dt>Present-state basis</dt><dd>{scenario.currentStateEstimate ? `${scenario.currentStateEstimate.asOfDate} · low-confidence illustrative hypothesis` : "Illustrative · not empirically calibrated"}</dd></div></dl>}<strong className="launch-link">Open simulation <span>→</span></strong></div></button>;
}
...
function SimulationLab({ scenario, protocol, interventionPlan, params, updateParam, scheduled, frames, summary, exportConfiguration, importConfiguration, announce }: { scenario: ScenarioDefinition; protocol: ScenarioProtocolDefinition; interventionPlan: InterventionPlanDefinition; params: SimulationParameters; updateParam: (key: keyof SimulationParameters, value: number) => void; scheduled: ScheduledIntervention[]; frames: SimulationFrame[]; summary: ReturnType<typeof simulate>["summary"]; exportConfiguration: () => void; importConfiguration: () => void; announce: (message: string) => void }) {
  ...
  return <div className="page-view">
    <section className="page-hero compact"><div className="eyebrow"><span>Research mode</span><span>Model {MODEL_VERSION}</span><span>{maintenancePatterns.length} maintenance patterns</span><span>{interventionDefinitions.length} available changes</span></div><h1>Simulation Laboratory</h1><p>Choose a bounded system, scenario, and response to test; then run deterministic simulations and inspect the result.</p></section>
    <div className="lab-layout">
      <Panel title="Run configuration" subtitle={scenario.title}>
        ...
        <p className="lab-protocol-note">Simulation setup: {maintenancePatternById[scenario.system.maintenancePatternId].title} → {scenario.system.shortTitle} → {protocol.title} → {interventionPlan.title} · {batchSize} nearby deterministic seeds · {scheduled.length} planned change{scheduled.length === 1 ? "" : "s"} retained in every run.</p>
      </Panel>
      ...
      <Panel className="full-span" title="What you can vary" subtitle="Explore maintenance patterns, scenarios, and planned responses independently"><div className="module-registry-grid"><section><h3>Maintenance patterns</h3>{maintenancePatterns.map((item) => <article key={item.id}><strong>{item.title}</strong><small>{item.summary}</small></article>)}</section><section><h3>Available scenarios</h3>{scenario.protocols.map((item) => <article key={item.id}><strong>{item.title}</strong><small>{item.kind} · {item.learningObjective}</small></article>)}</section><section><h3>Planned responses</h3>{interventionPlans.map((item) => <article key={item.id}><strong>{item.title}</strong><small>{item.strategy} · {item.items.length} change{item.items.length === 1 ? "" : "s"}</small></article>)}</section></div></Panel>
      <Panel className="full-span" title="Published systems" subtitle={`${scenarios.length} bounded systems; each watchlist outlook comes from the system's dated default scenario`}><div className="table-wrap"><table><thead><tr><th>System</th><th>Domain</th><th>Operator</th><th>Default scenario</th><th>Derived outlook</th><th>Maintenance pattern</th><th>Dynamic traits</th><th>Version</th><th>Estimate basis</th></tr></thead><tbody>{scenarios.map((item) => <tr key={item.id}><td>{item.system.title}</td><td>{item.system.domain}</td><td>{item.system.operator}</td><td>{item.protocols.find((protocol) => protocol.id === item.defaultProtocolId)?.title}</td><td>{item.watchlistTier}</td><td>{maintenancePatternById[item.maintenancePatternId].title}</td><td>{item.system.dynamicTraits.join(", ")}</td><td>{item.version}</td><td>{item.currentStateEstimate ? `${item.currentStateEstimate.asOfDate} · ${item.currentStateEstimate.confidence} confidence` : "Illustrative · not empirically calibrated"}</td></tr>)}</tbody></table></div></Panel>
    </div>
  </div>;
}
