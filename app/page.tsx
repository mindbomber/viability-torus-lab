...
export default function Home() {
  ...
  const currentFrame = run.frames[Math.min(frameIndex, run.frames.length - 1)] ?? run.frames[0];
  ...
  const applyIntervention = (definition: (typeof interventions)[number]) => {
    const event: ScheduledIntervention = {
      ...
      step: Math.min(frameIndex + 1, params.steps - 1),
      effects: definition.build(params),
      cost: definition.cost,
    };
    setScheduled((items) => [...items, event]);
    setPlaying(true);
    track("intervention_applied", { intervention: definition.id, step: frameIndex });
    announce(`${definition.label} scheduled at t=${currentFrame.time.toFixed(1)}`);
  };
  ...
  const loadSavedPreset = () => {
    try {
      ...
      setParams({ ...defaultParameters, ...saved.params });
      setScheduled([]);
      setFrameIndex(0);
      announce("Saved preset loaded");
    } catch {
      announce("No valid saved preset found");
    }
  };
  ...
  const shareRun = async () => {
    ...
    url.searchParams.set("pi", params.pressure.toFixed(2));
    url.searchParams.set("gamma", params.feedback.toFixed(2));
    url.searchParams.set("c", params.correction.toFixed(2));
    url.searchParams.set("epsilon", params.error.toFixed(2));
    url.searchParams.set("debt", params.initialDebt.toFixed(2));
  ...
  const importConfiguration = async (file: File) => {
    try {
      ...
      if (!data?.configuration?.parameters || !scenarioById[data.configuration.scenarioId]) throw new Error("Invalid configuration schema");
      const imported = { ...defaultParameters, ...data.configuration.parameters } as SimulationParameters;
      if (imported.steps < 1 || imported.steps > 10000 || imported.feedback < 0 || imported.feedback > 1) throw new Error("Parameter values are out of range");
      setScenarioId(data.configuration.scenarioId);
      setParams(imported);
      setScheduled(Array.isArray(data.configuration.interventions) ? data.configuration.interventions.slice(0, 100) : []);
      setFrameIndex(0);
      setPlaying(false);
  ...
  return (
    <div className={`app-shell ${highContrast ? "high-contrast" : ""}`}>
      ...
      <aside className={`sidebar ${mobileNav ? "open" : ""}`} aria-label="Primary navigation">
        ...
        <div className="lab-card"><strong>Toroidal Systems Lab</strong><span>Research-grade simulations for viability and alignment</span><div><small>v1.0.0</small><small><i /> Online</small></div></div>
      </aside>
      {mobileNav && <button className="nav-scrim" aria-label="Close navigation" onClick={() => setMobileNav(false)} />}

      <div className="app-main">
        <header className="topbar">
          <button className="mobile-menu" onClick={() => setMobileNav((v) => !v)} aria-expanded={mobileNav} aria-controls="primary-nav">☰<span>Menu</span></button>
        ...
        <main id="main-content" tabIndex={-1}>
          ...
          {view === "compare" && <CompareMode scenario={scenario} params={params} primaryFrames={run.frames} primarySummary={run.summary} selectScenario={selectScenario} />}
          {view === "builder" && <SystemBuilder announce={announce} />}
          {view === "learn" && <LearnSection />}
          {view === "theory" && <TheorySection announce={announce} />}
        </main>
        <footer className="site-footer"><span>Model {MODEL_VERSION} · Synthetic model behavior, not empirical validation.</span><button onClick={() => setHighContrast((v) => !v)} aria-pressed={highContrast}>◐ High contrast</button><a href="/paper.pdf" download>Download paper</a></footer>
      </div>
...
function SimulatorView(props: SimulatorProps) {
  const { scenario, params, frames, frameIndex, setFrameIndex, playing, scheduled, mode } = props;
  const frame = frames[Math.min(frameIndex, frames.length - 1)];
  const result = useMemo(() => simulate(params, scheduled), [params, scheduled]);
  ...
  return (
    <div className="dashboard-view">
      <section className="scenario-title-row">
        <div><div className="eyebrow"><span>{scenario.category}</span><span>{scenario.difficulty}</span><span>Scenario v1.0</span></div><h1>{scenario.title}</h1><p>{scenario.summary}</p></div>
        <div className="scenario-facts"><span><small>Optimizes</small>{scenario.optimizedOutcome}</span><span><small>Current seed</small>{params.seed}</span></div>
      </section>
      <div className="dashboard-grid">
        ...
        <Panel className="parameter-panel" title="System Parameters" subtitle={mode === "guided" ? "Plain-language controls" : "Canonical ATS/AANA/AIx variables"} action={<button className="text-button" onClick={props.restoreDefaults}>Reset</button>}>
          <div className="preset-row"><select aria-label="Load scenario preset" defaultValue=""><option value="" disabled>Load preset…</option>{scenario.presets.map((preset) => <option key={preset.name} value={preset.name}>{preset.name}</option>)}</select>{scenario.presets.map((preset) => <button key={preset.name} onClick={() => props.setParams((current) => ({ ...current, ...preset.values }))}>{preset.name}</button>)}</div>
          <div className="parameter-list">
            {visibleParameters.map((key) => <ParameterControl key={key} parameterKey={key} label={scenario.labels[key]} value={params[key]} meta={parameterMeta[key]} locked={props.locked.includes(key)} onLock={() => props.toggleLock(key)} onChange={(value) => props.updateParam(key, value)} />)}
          </div>
          {mode === "research" && <details className="advanced-params" open><summary>Advanced dynamics</summary><div className="advanced-grid">
            <NumberField label="κ restoration" value={params.kappa} step={.01} onChange={(value) => props.updateParam("kappa", value)} />
            <NumberField label="χ debt pressure" value={params.chi} step={.01} onChange={(value) => props.updateParam("chi", value)} />
            <NumberField label="ωθ local frequency" value={params.omegaTheta} step={.005} onChange={(value) => props.updateParam("omegaTheta", value)} />
            <NumberField label="ωφ external frequency" value={params.omegaPhi} step={.005} onChange={(value) => props.updateParam("omegaPhi", value)} />
            <NumberField label="ρ₀ reference" value={params.rho0} step={.05} onChange={(value) => props.updateParam("rho0", value)} />
            <NumberField label="ρcrit threshold" value={params.rhoCrit} step={.05} onChange={(value) => props.updateParam("rhoCrit", value)} />
          </div></details>}
        </Panel>
        <div className="status-column">
          <Panel className="status-panel" title="System Status" subtitle={`t = ${frame.time.toFixed(1)} · step ${frame.step}`}>
            ...
            <div className="status-mini-grid"><span><small>Radial velocity</small>{signed(frame.radialVelocity)}</span><span><small>Divergence</small>{frame.divergence.toFixed(3)}</span><span><small>Minor phase</small>{phaseName(frame.theta, scenario.cycles.minor.stages)}</span><span><small>Major phase</small>{phaseName(frame.phi, scenario.cycles.major.stages)}</span></div>
          </Panel>
      ...
      <details className="data-table-fallback"><summary>Accessible data table for the current run</summary><div className="table-wrap"><table><caption>Latest simulation frames, linked to the charts above</caption><thead><tr><th>Step</th><th>Time</th><th>Alignment</th><th>Excursion</th><th>Debt</th><th>Margin</th><th>Status</th></tr></thead><tbody>{frames.slice(Math.max(0, frameIndex - 12), frameIndex + 1).map((item) => <tr key={item.step}><td>{item.step}</td><td>{item.time.toFixed(2)}</td><td>{item.alignment.toFixed(3)}</td><td>{item.rho.toFixed(3)}</td><td>{item.debt.toFixed(3)}</td><td>{item.correctionMargin.toFixed(3)}</td><td>{item.status}</td></tr>)}</tbody></table></div></details>
    </div>
  );
}
...
function ParameterControl({ parameterKey, label, value, meta, locked, onLock, onChange }: { parameterKey: ParameterKey; label: string; value: number; meta: (typeof parameterMeta)[ParameterKey]; locked: boolean; onLock: () => void; onChange: (value: number) => void }) {
  return <div className={`parameter-control tone-${meta.color} ${locked ? "locked" : ""}`}><div className="parameter-label"><span><strong>{label}</strong><small>{meta.symbol} · {meta.description}</small></span><button onClick={onLock} aria-label={`${locked ? "Unlock" : "Lock"} ${label}`} aria-pressed={locked}>{locked ? "▣" : "▢"}</button></div><div className="slider-row"><input type="range" aria-label={`${label}, ${meta.symbol}`} min={meta.min} max={meta.max} step={meta.step} value={value} disabled={locked} onChange={(event) => onChange(Number(event.target.value))} /><input aria-label={`${label} numeric value`} type="number" min={meta.min} max={meta.max} step={meta.step} value={value} disabled={locked} onChange={(event) => onChange(Math.max(meta.min, Math.min(meta.max, Number(event.target.value))))} /></div></div>;
}
...
function ScenarioCard({ scenario, onClick, compact = false, active = false }: { scenario: ScenarioDefinition; onClick: () => void; compact?: boolean; active?: boolean }) {
  return <button className={`scenario-card ${compact ? "compact" : ""} ${active ? "active" : ""}`} onClick={onClick} style={{ "--scenario-accent": scenario.accent } as React.CSSProperties}><div className="scenario-art"><span>{scenario.icon}</span><i /><i /><i /></div><div className="scenario-card-body"><div><span className="category-pill">{scenario.category}</span><span>{scenario.difficulty}</span></div><h3>{scenario.shortTitle}</h3><p>{scenario.summary}</p>{!compact && <dl><div><dt>Minor cycle</dt><dd>{scenario.cycles.minor.label}</dd></div><div><dt>Major cycle</dt><dd>{scenario.cycles.major.label}</dd></div><div><dt>Debt</dt><dd>{scenario.debtMechanism}</dd></div></dl>}<strong className="launch-link">Launch scenario <span>→</span></strong></div></button>;
}
...
function SimulationLab({ scenario, params, setParams, frames, summary, exportConfiguration, importConfiguration, announce }: { scenario: ScenarioDefinition; params: SimulationParameters; setParams: (p: SimulationParameters) => void; frames: SimulationFrame[]; summary: ReturnType<typeof simulate>["summary"]; exportConfiguration: () => void; importConfiguration: () => void; announce: (message: string) => void }) {
  ...
  return <div className="page-view"><section className="page-hero compact"><div className="eyebrow"><span>Research mode</span><span>Model {MODEL_VERSION}</span></div><h1>Simulation Laboratory</h1><p>Configure deterministic runs, execute seeded ensembles, inspect model metadata, and export reproducible evidence.</p></section><div className="lab-layout"><Panel title="Run configuration" subtitle={scenario.title}><div className="form-grid"><NumberField label="Seed" value={params.seed} step={1} onChange={(value) => setParams({ ...params, seed: Math.round(value) })} /><NumberField label="Steps (≤10,000)" value={params.steps} step={100} onChange={(value) => setParams({ ...params, steps: Math.max(1, Math.min(10000, Math.round(value))) })} /><NumberField label="Integration step Δt" value={params.dt} step={.05} onChange={(value) => setParams({ ...params, dt: Math.max(.01, value) })} /><NumberField label="Ensemble seeds" value={batchSize} step={1} onChange={(value) => setBatchSize(Math.max(2, Math.min(100, Math.round(value))))} /></div><div className="button-row"><button className="primary" onClick={runBatch}>Run batch</button><button onClick={exportConfiguration}>Export configuration</button><button onClick={importConfiguration}>Import configuration</button><button onClick={() => downloadCsv(`${scenario.id}-timeseries.csv`, frames)}>Export CSV</button></div></Panel><Panel title="Current run summary" subtitle={`Seed ${params.seed}`}><div className="summary-cards"><SummaryStat label="Stable time" value={`${(summary.stableFraction * 100).toFixed(1)}%`} /><SummaryStat label="Max excursion" value={summary.maxRho.toFixed(3)} /><SummaryStat label="Final alignment" value={summary.finalAlignment.toFixed(3)} /><SummaryStat label="Winding ratio" value={summary.windingRatio.toFixed(3)} /><SummaryStat label="First warning" value={summary.firstWarningStep?.toString() ?? "None"} /><SummaryStat label="Rupture" value={summary.ruptureStep?.toString() ?? "Prevented"} /></div></Panel>{batch.length > 0 && <Panel className="full-span" title="Ensemble result" subtitle={`${batch.length} deterministic seeds`}><div className="ensemble-strip"><SummaryStat label="Mean final alignment" value={avg("finalAlignment").toFixed(3)} /><SummaryStat label="Mean final debt" value={avg("finalDebt").toFixed(3)} /><SummaryStat label="Mean max excursion" value={avg("maxRho").toFixed(3)} /><SummaryStat label="Rupture probability" value={`${(batch.filter((item) => item.ruptureStep !== undefined).length / batch.length * 100).toFixed(1)}%`} /></div></Panel>}<Panel className="full-span" title="Scenario registry" subtitle="Version-controlled administrative content"><div className="table-wrap"><table><thead><tr><th>Scenario</th><th>Category</th><th>Version</th><th>State</th><th>Parameters</th></tr></thead><tbody>{scenarios.map((item) => <tr key={item.id}><td>{item.title}</td><td>{item.category}</td><td>1.0.0</td><td><span className="published">● Published</span></td><td>{Object.keys(item.defaults).length} mapped</td></tr>)}</tbody></table></div></Panel></div></div>;
}

function CompareMode({ scenario, params, primaryFrames, primarySummary, selectScenario }: { scenario: ScenarioDefinition; params: SimulationParameters; primaryFrames: SimulationFrame[]; primarySummary: ReturnType<typeof simulate>["summary"]; selectScenario: (scenario: ScenarioDefinition) => void }) {
  const [compareScenarioId, setCompareScenarioId] = useState(scenario.id);
  const [compareParams, setCompareParams] = useState({ ...params, feedback: Math.max(0, params.feedback - .18), correction: Math.max(0, params.correction - .14), pressure: Math.min(3, params.pressure + .3) });
  const comparison = useMemo(() => simulate(compareParams), [compareParams]);
  ...
  return <div className="page-view"><section className="page-hero compact"><div className="eyebrow"><span>Synchronized comparison</span><span>Seed {params.seed}</span></div><h1>Compare two futures</h1><p>Hold the scenario and seed constant, then isolate the effect of feedback, correction, pressure, or intervention timing.</p></section><div className="compare-controls"><label>Comparison scenario<select value={compareScenarioId} onChange={(event) => { const next = scenarioById[event.target.value]; setCompareScenarioId(next.id); setCompareParams(next.defaults); }}>{scenarios.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label>Comparison correction C<input type="range" min={0} max={1.5} step={.01} value={compareParams.correction} onChange={(event) => setCompareParams({ ...compareParams, correction: Number(event.target.value) })} /><strong>{compareParams.correction.toFixed(2)}</strong></label><label>Comparison feedback γ<input type="range" min={0} max={1} step={.01} value={compareParams.feedback} onChange={(event) => setCompareParams({ ...compareParams, feedback: Number(event.target.value) })} /><strong>{compareParams.feedback.toFixed(2)}</strong></label></div><div className="compare-grid"><Panel title="A · Active configuration" subtitle={`${scenario.shortTitle} · C ${params.correction.toFixed(2)} · γ ${params.feedback.toFixed(2)}`}><TorusCanvas compact frames={primaryFrames} frameIndex={index} params={params} playing={false} onSelectFrame={() => {}} /><CompareSummary summary={primarySummary} /></Panel><Panel title="B · Comparison configuration" subtitle={`${scenarioById[compareScenarioId].shortTitle} · C ${compareParams.correction.toFixed(2)} · γ ${compareParams.feedback.toFixed(2)}`}><TorusCanvas compact frames={comparison.frames} frameIndex={index} params={compareParams} playing={false} onSelectFrame={() => {}} /><CompareSummary summary={comparison.summary} /></Panel><Panel className="full-span" title="Outcome difference · A minus B" subtitle="Same model; controlled parameter difference"><div className="difference-cards"><SummaryStat label="Δ alignment" value={signed(delta.alignment)} tone={delta.alignment >= 0 ? "good" : "bad"} /><SummaryStat label="Δ max excursion" value={signed(delta.rho)} tone={delta.rho <= 0 ? "good" : "bad"} /><SummaryStat label="Δ debt" value={signed(delta.debt)} tone={delta.debt <= 0 ? "good" : "bad"} /><SummaryStat label="Time to warning" value={`${primarySummary.firstWarningStep ?? "∞"} / ${comparison.summary.firstWarningStep ?? "∞"}`} /></div><TimeSeriesChart frames={differenceFrames(primaryFrames, comparison.frames)} frameIndex={index} params={params} label="Normalized difference chart for alignment, debt, and radial excursion" /><p className="compare-conclusion">{delta.alignment > .04 ? "Configuration A preserves meaningfully more alignment." : Math.abs(delta.alignment) < .02 ? "The runs finish with similar alignment, but their debt and recovery requirements may differ." : "Configuration B preserves more alignment under these settings."}</p></Panel></div></div>;
}

function CompareSummary({ summary }: { summary: ReturnType<typeof simulate>["summary"] }) { return <div className="compare-summary"><SummaryStat label="Final A" value={summary.finalAlignment.toFixed(3)} /><SummaryStat label="Final Δ" value={summary.finalDebt.toFixed(3)} /><SummaryStat label="Max ρ" value={summary.maxRho.toFixed(3)} /><SummaryStat label="Outcome" value={summary.ruptureStep ? "Ruptured" : summary.recovered ? "Recovered" : "Viable"} /></div>; }
...
function statusToneFor(status: string) { if (status === "Stable" || status === "Phase locked") return "stable"; if (status === "Ruptured" || status === "Rupture approaching") return "danger"; if (status === "Recovering") return "recovering"; return "warning"; }
function statusMessage(frame: SimulationFrame) { if (frame.status === "Stable") return "System is inside the viable tube."; if (frame.status === "Recovering") return "Excursion is contracting after stress."; if (frame.status === "Ruptured") return "Critical radial boundary crossed."; if (frame.status === "Phase locked") return "Recurrent phases are synchronized."; return frame.correctionMargin < 0 ? "Divergence exceeds current correction." : "Viability margin is narrowing."; }
function phaseName(angle: number, stages: string[]) { return stages[Math.floor((angle / (Math.PI * 2)) * stages.length) % stages.length]; }
...
function configurationPayload(scenario: ScenarioDefinition, params: SimulationParameters, interventions: ScheduledIntervention[], summary: ReturnType<typeof simulate>["summary"]) { return { exportedAt: new Date().toISOString(), modelVersion: MODEL_VERSION, scenarioVersion: "1.0.0", configuration: { scenarioId: scenario.id, seed: params.seed, steps: params.steps, dt: params.dt, parameters: params, events: [], interventions }, summary }; }
...
function downloadCsv(filename: string, frames: SimulationFrame[]) { const header = "step,time,theta,phi,rho,debt,alignment,divergence,correction,correctionMargin,irreversibleLoss,status"; const rows = frames.map((f) => [f.step, f.time, f.theta, f.phi, f.rho, f.debt, f.alignment, f.divergence, f.correction, f.correctionMargin, f.irreversibleLoss, f.status].join(",")); downloadText(filename, [header, ...rows].join("\n"), "text/csv"); }
