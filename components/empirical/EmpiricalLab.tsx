...
export function EmpiricalLab({ scenario, announce, onRegisterEvidence }: Props) {
  ...
  const buildEvidenceBundle = (): ParsedEmpiricalEvidenceBundle | null => {
    ...
    return empiricalEvidenceBundleSchema.parse({
      ...
      limitations: [
        "The source data remain local and are not included in this record; the SHA-256 fingerprint covers the standardized table used by the lab, not the original file bytes.",
        "Required variables are declared proxies on documented scales; this first release does not infer or fit real-world rescaling rules or model parameters.",
  ...
  const exportEvidence = () => {
    ...
    announce("Browser-local study record exported without raw observations");
  };
...
function MappingStep({ dataset, mapping, setMappingEntry, analysis, next }: { dataset: EmpiricalDataset | null; mapping: EmpiricalColumnMapping; setMappingEntry: (role: EmpiricalRole, patch: Partial<EmpiricalColumnMapping[EmpiricalRole]>) => void; analysis: EmpiricalStudyAnalysis | null; next: () => void }) {
  ...
  return <section className="empirical-step"><header><div><span>Connect your data</span><h2>Match observed columns to the paper variables</h2></div><p>You choose each connection. The lab does not silently rescale real-world units onto model scales.</p></header><MappingTable dataset={dataset} mapping={mapping} setMappingEntry={setMappingEntry} /><section className={`empirical-mapping-receipt ${analysis?.issues.length ? "has-issues" : "ready"}`}><strong>{analysis?.issues.length ? "Column setup needs attention" : "Column setup is ready for evidence checks"}</strong>{analysis?.issues.length ? <ul>{analysis.issues.slice(0, 8).map((issue) => <li key={issue}>{issue}</li>)}</ul> : <p>Each required variable uses a distinct column, includes units, and falls within the documented model range.</p>}</section><footer><button className="primary" onClick={next}>Run evidence checks →</button></footer></section>;
}
...
function ExportStep({ dataset, datasetHash, analysis, mapping, exportEvidence, registerEvidence, canRegister }: { dataset: EmpiricalDataset | null; datasetHash: string; analysis: EmpiricalStudyAnalysis | null; mapping: EmpiricalColumnMapping; exportEvidence: () => void; registerEvidence: () => void; canRegister: boolean }) {
  if (!dataset || !analysis) return <EmptyEmpiricalState title="No study record is available" copy="Complete at least the data-quality check before exporting a browser-local study record." />;
  return <section className="empirical-step"><header><div><span>Reproducible study record</span><h2>Export evidence without exporting raw observations</h2></div><p>The JSON file records the standardized-table fingerprint, column connections, declared assumptions, evidence checks, holdout metrics when available, model version, and limitations. Failed checks remain exportable negative evidence. Keep the original CSV separately.</p></header><div className="empirical-export-grid"><section><h3>Standardized table fingerprint</h3><code>{datasetHash || "Computing SHA-256…"}</code><dl><div><dt>Rows</dt><dd>{dataset.rows.length}</dd></div><div><dt>Columns</dt><dd>{dataset.headers.length}</dd></div><div><dt>Raw data included</dt><dd>No</dd></div><div><dt>Browser-local</dt><dd>Yes</dd></div></dl></section><section><h3>Evidence classification</h3><dl><div><dt>Evidence level</dt><dd>Observed descriptive</dd></div><div><dt>Model support</dt><dd>{analysis.modelSupport}</dd></div><div><dt>Torus replay</dt><dd>{analysis.torusReplayReady ? "Provisional" : "Withheld"}</dd></div><div><dt>Required variables</dt><dd>{mappingUnitsDocumented(mapping) ? "Documented" : "Incomplete"}</dd></div></dl></section>{analysis.replay && <section><h3>Holdout results</h3><dl><div><dt>Calibration rows</dt><dd>{analysis.replay.calibrationRows}</dd></div><div><dt>Holdout rows</dt><dd>{analysis.replay.holdoutRows}</dd></div><div><dt>Holdout RMSE</dt><dd>{analysis.replay.holdoutRmse.toFixed(4)}</dd></div><div><dt>Interval coverage</dt><dd>{(analysis.replay.holdoutIntervalCoverage * 100).toFixed(1)}%</dd></div></dl></section>}</div><section className="empirical-export-boundary"><strong>What this study record does not claim</strong><p>No parameter fit, domain calibration, causal effect, universal threshold, or empirical validation of the foundational theory is asserted.</p></section><footer><button disabled={!datasetHash || !canRegister} onClick={registerEvidence}>Add to Evidence Registry</button><button className="primary" disabled={!datasetHash} onClick={exportEvidence}>Export evidence file</button></footer></section>;
}
