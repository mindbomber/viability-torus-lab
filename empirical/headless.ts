...
function buildReceipt(
  ...
  return empiricalResearchReceiptSchema.parse({
    ...
    evidence: {
      ...
      modelSupport: analysis.modelSupport,
      interpretationBoundary: "This receipt records observed-descriptive model evaluation. It does not establish causal identification, domain calibration, toroidal topology, or empirical validation of ATS, AANA, or AIx.",
    },
    validation: validationReceipt(analysis),
    replay: replayReceipt(analysis.replay),
    limitations: [
      "Raw observations are excluded from this receipt; the fingerprint covers the canonical parsed table, not the original source bytes.",
